...
import           Text.Mustache          (Template, automaticCompile, object,
                                         substitute, (~>))
import           Text.Mustache.Types    (Value)
import           Web.Core               (WRAction, WRConfig (wrcTplRoots),
                                         WRState (wrstMainTemplate), runSqlite,
                                         wrconUser)
...
weightGraphValue wrs = do
  ...
  where
    flat wr = do
      ...
      let
        ztimeStr = TM.formatTime TM.defaultTimeLocale "%m/%d" ztime
        w = WRecord.weight wr
      return (ztimeStr, w)
    groupToValue gr =
      let
